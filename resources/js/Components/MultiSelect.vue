...
function toggle() {
    ...
    if (open.value) {
        nextTick(() => {
            // Detect viewport overflow: if dropdown would extend past right edge,
            // anchor it to the right of the trigger instead of the left.
            const rect = rootEl.value?.getBoundingClientRect();
            if (rect) {
                const dropdownWidth = 224; // w-56
                const margin        = 8;
                alignRight.value = (rect.left + dropdownWidth + margin) > window.innerWidth;
            }
...
<template>
    <div ref="rootEl" class="relative">
        <button
            type="button"
            @click="toggle"
            class="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-semibold rounded-lg border transition-colors min-h-[36px]"
            :class="count > 0
        ...
        >
            <div
                v-if="open"
                class="absolute z-30 mt-1.5 w-56 max-w-[calc(100vw-2rem)] max-h-72 overflow-hidden bg-white border border-slate-200 rounded-xl shadow-lg flex flex-col"

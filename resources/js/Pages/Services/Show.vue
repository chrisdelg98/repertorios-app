...
const teamManualRoleId = ref(null);
...
async function submitManualAssignment() {
    ...
    if (!teamManualRoleId.value) {
        assignmentError.value = t('assignments.errors.missing_role');
        return;
    }
    const success = await saveAssignment({
        manual_name: teamManualName.value.trim(),
        band_role_type_id: Number(teamManualRoleId.value),
    });
    if (success) {
        teamManualName.value = '';
        teamManualRoleId.value = null;
        ...
        <Teleport to="body">
            ...
            <div v-if="showTeamSheet" class="fixed bottom-0 left-1/2 lg:left-[calc(50%+8rem)] -translate-x-1/2 w-full max-w-lg lg:max-w-3xl z-50 bg-white rounded-t-2xl px-4 lg:px-6 pt-4 pb-8 max-h-[85vh] flex flex-col shadow-xl">
                ...
                <div class="overflow-y-auto flex-1">
                    ...
                    <div
                        v-if="canManageAssignments && teamComposerOpen"
                    ...
                    >
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button
                                type="button"
                                @click="teamComposerOpen = false; resetTeamComposer(); assignmentError = ''"
                                class="w-7 h-7 inline-flex items-center justify-center rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors"
                            >
                        ...
                        <button
                            type="button"
                            @click="teamManualOpen = !teamManualOpen"
                            class="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-semibold text-indigo-600 bg-white hover:bg-indigo-50 border border-indigo-200 rounded-lg transition-colors"
                        >
                            <svg class="w-4 h-4 transition-transform" :class="teamManualOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                            </svg>
                        ...
                        <div v-if="teamManualOpen" class="mt-2.5 border border-slate-200 rounded-xl p-3 space-y-2.5">
                            ...
                            <button
                                ...
                                :disabled="assignmentsProcessing || !teamManualName.trim() || !teamManualRoleId"
                                class="w-full py-2.5 text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg disabled:opacity-40"
                            >
                                {{ t('assignments.add_button') }}
                            </button>
                ...
                <button
                    v-if="canManageAssignments && localAssignments.length && !teamComposerOpen"
                    type="button"
                    @click="openTeamComposer"

...
<template>
    ...
    <div class="relative min-h-screen bg-slate-50 flex flex-col">
        ...
        <div v-if="expired" class="flex-1 flex flex-col items-center justify-center px-6 text-center">
            ...
            <p class="text-sm text-slate-600">{{ t('public.expired_body') }}</p>
        </div>
        ...
        <template v-else-if="service">
            ...
            <div v-if="hasAssignments" class="bg-white border-b border-slate-100 px-5 py-4">
                <p class="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">{{ t('assignments.section_title') }}</p>
            ...
            <div class="flex-1 px-4 py-5">
                <div v-if="!service.songs.length" class="text-center py-12 text-sm text-slate-600">
                    {{ t('public.no_songs') }}
                </div>
                ...
                <div v-if="service.songs.length" class="space-y-2">
                    ...
                    >
                        <span class="text-xs font-bold text-slate-400 w-5 text-center shrink-0">{{ i + 1 }}</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-slate-900 truncate">{{ song.name }}</p>
                            <p class="text-xs font-medium text-slate-600 mt-0.5">
                                <span v-if="song.artist" class="text-slate-600">{{ song.artist }} · </span>{{ song.version }}<span v-if="song.key" class="text-indigo-600 font-semibold"> · {{ song.key }}</span>
                            </p>
                        </div>
